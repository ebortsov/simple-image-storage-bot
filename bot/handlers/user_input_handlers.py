...
async def upload_name_enter(message: types.Message, db_conn: sqlite3.Connection):
...
async def upload_photo(message: types.Message, bot: Bot, db_conn: sqlite3.Connection):
    user_id = message.from_user.id
    try:
...
async def delete_name_enter(message: types.Message, db_conn: sqlite3.Connection):
    try:
        user_id = message.from_user.id
        photoname = await utils.handle_raw_photoname_check(message)
        if not photoname:
...
async def show_photo(message: types.Message, db_conn: sqlite3.Connection):
    try:
        user_id = message.from_user.id
        photoname = await utils.handle_raw_photoname_check(message)
        if not photoname:
